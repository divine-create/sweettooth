...
// Script to inflate all items and products stock to 5,000 for testing across all branches
...
echo "Starting stock inflation to 5,000 for all items and products...\n";
...
foreach ($stocks as $stock) {
    $stock->update([
        ...
        'total_available' => 5000, // In case there's a total_available field
        'closing_quantity' => 5000, // In case there's a closing_quantity field
        'notes' => $stock->notes . ' | Stock inflated to 5000 on ' . Carbon::now()
    ]);
    $itemStocksUpdated++;
}
...
foreach ($productStocks as $productStock) {
    $productStock->update([
        ...
        'glovo_quantity' => 0,
        'quantity_sold' => 0,
        'closing_quantity' => 5000,
        'notes' => $productStock->notes . ' | Stock inflated to 5000 on ' . Carbon::now()
    ]);
...
foreach ($itemsWithoutStock as $item) {
    ...
    Stock::create([
        ...
        'health_status' => 'good',
        'notes' => 'Initial stock inflated to 5000 on ' . Carbon::now()
    ]);
    $itemsAddedStock++;
}
