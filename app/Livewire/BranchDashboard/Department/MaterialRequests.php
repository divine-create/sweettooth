...
namespace App\Livewire\BranchDashboard\Department;
...
class MaterialRequests extends Component
{
    ...
    public function mount()
    {
        ...
        if (!$actor?->department_id) {
            abort(403, 'You must be assigned to a department to make material requests.');
        }
    }
    ...
    public function render()
    {
        ...
        return view('livewire.branch-dashboard.department.material-requests', [
            'requests' => $requests,
        ]);
    }
    ...
    public function saveRequest()
    {
        ...
        $branchId = $this->getBranchId();
        $actor = current_actor();
        $targetDept = Department::find($actor?->department_id);

        if (!$targetDept) {
            $this->toast()->error('You are not assigned to a department.')->send();
            return;
        }
