...
namespace App\Livewire\BranchDashboard\SalesDashboard\Transactions;
...
class Index extends BaseComponent
{
    ...
    {
        $this->dialog()
            ->question('Warning!', 'Are you sure you want to delete ' . count($this->selectedIds) . ' sale(s)?')
            ->confirm('Confirm Delete', 'bulkDelete', count($this->selectedIds) . ' sale(s) deleted successfully!')
    ...
    protected function exportSelected(): void
    {
        if (empty($this->selectedIds)) {
            session()->flash('info', 'No sales selected for export.');
            return;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.branch-dashboard.sales-dashboard.transactions.index', [
