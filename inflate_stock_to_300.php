...
// Script to inflate all items and products stock to 300
...
echo "Starting stock inflation to 300 for all items and products...\n";
...
foreach ($stocks as $stock) {
    $stock->update([
        ...
        'total_available' => 300, // In case there's a total_available field
        'closing_quantity' => 300, // In case there's a closing_quantity field
        'notes' => $stock->notes . ' | Stock inflated to 300 on ' . Carbon::now()
    ]);
    $itemStocksUpdated++;
}
...
foreach ($productStocks as $productStock) {
    $productStock->update([
        ...
        'glovo_quantity' => 0,
        'quantity_sold' => 0,
        'closing_quantity' => 300,
        'notes' => $productStock->notes . ' | Stock inflated to 300 on ' . Carbon::now()
    ]);
...
// Note: ProductStock records are not directly linked to Product model via relationship
