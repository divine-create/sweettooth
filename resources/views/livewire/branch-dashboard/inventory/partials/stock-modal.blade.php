...
<div x-data="{ 
    ...
    <div x-show="show" x-transition:enter="transform transition ease-in-out duration-200"
        ...
        <div
            ...
            @if($stockItemId)
                @php
                    $branchId = $this->getBranchId() ?? current_branch_id() ?? request()->query('b_id');
                    $item = \App\Models\Item::with(['unitOfMeasure'])->where('id', $stockItemId)->first();
                    $stock = $item ? \App\Models\Stock::where('item_id', $item->id)
                ...
                @if($item)
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-zinc-900 dark:text-zinc-100">
                                    {{ $item->unitOfMeasure?->symbol ? strtoupper($item->unitOfMeasure->symbol) : 'N/A' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-3">Current Stock Quantities</h3>

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <div class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4">
                                <div class="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Available</div>
                                <div class="text-xl font-bold text-zinc-900 dark:text-zinc-100">
                                    {{ number_format($stock->quantity_available ?? 0, 2) }}
                                </div>
                            </div>
                            <div class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4">
                                <div class="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Reserved</div>
                                <div class="text-xl font-bold text-zinc-900 dark:text-zinc-100">
                                    {{ number_format($stock->quantity_reserved ?? 0, 2) }}
                                </div>
                            </div>
                            <div class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4">
                                <div class="text-xs text-zinc-500 dark:text-zinc-400 mb-1">Damaged</div>
                                <div class="text-xl font-bold text-zinc-900 dark:text-zinc-100">
                                    {{ number_format($stock->quantity_damaged ?? 0, 2) }}
                                </div>
                            </div>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 transition-all duration-200">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-xl font-bold text-blue-900 dark:text-blue-100">
                                    {{ number_format(($stock->quantity_available ?? 0) + ($stock->quantity_reserved ?? 0) + ($stock->quantity_damaged ?? 0), 2) }}
                                </span>
                            </div>
                        </div>
                    </div>
        ...
        <div
            ...
            <a href="{{ branch_route('branch-dashboard.inventory.stocks') }}"
