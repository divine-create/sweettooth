    ...
    <div class="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-semibold text-zinc-900 dark:text-white">Debit Notes</h1>
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Raise debit notes to suppliers for returns or overcharges.</p>
            </div>
        ...
        <table class="min-w-full divide-y divide-zinc-100 dark:divide-zinc-800 text-sm">
            ...
            <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800">
                ...
                <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/40">
                    <td class="px-4 py-3 font-mono text-xs text-zinc-700 dark:text-zinc-300">{{ $note->debit_note_number }}</td>
                    <td class="px-4 py-3 text-zinc-800 dark:text-zinc-200">{{ $note->supplier?->name ?? '—' }}</td>
                    <td class="px-4 py-3 text-zinc-600 dark:text-zinc-400">{{ $note->debit_note_date->format('d M Y') }}</td>
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-4 py-10 text-center text-sm text-zinc-400">No debit notes found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 pt-16">
        <div class="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
            <div class="mb-5 flex items-center justify-between">
                <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">New Debit Note</h2>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs font-semibold uppercase tracking-wide text-zinc-500">Supplier *</label>
                    ...
                    <div>
                        ...
                        <input type="date" wire:model.defer="debit_note_date"
                            class="mt-1 w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white" />
                        @error('debit_note_date') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>

                @if ($supplier_id && $supplierPurchases->isNotEmpty())
                <div>
                    <label class="text-xs font-semibold uppercase tracking-wide text-zinc-500">Linked Purchase (optional)</label>
                    <select wire:model.defer="purchase_id"
                        class="mt-1 w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-950 dark:text-white">
                        <option value="">— No linked purchase —</option>
                        @foreach ($supplierPurchases as $purchase)
                        <option value="{{ $purchase->id }}">
                            {{ $purchase->purchase_number }} — {{ \Carbon\Carbon::parse($purchase->purchase_date)->format('d M Y') }}
                            ({{ \App\Helpers\LocalizationHelper::formatCurrency($purchase->total_cost) }})
                        </option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4 pt-16">
        <div class="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
            <div class="mb-4 flex items-center justify-between">
                <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">{{ $viewingNote->debit_note_number }}</h2>
            ...
            <dl class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <dd class="font-medium">{{ $viewingNote->supplier?->name ?? '—' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-zinc-500">Date</dt>
                    <dd class="font-medium">{{ $viewingNote->debit_note_date->format('d M Y') }}</dd>
                </div>
                <div class="flex justify-between">
