    ...
    @if(!$currentMonthExists)
        <div class="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-800 dark:bg-amber-900/20">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-amber-800 dark:text-amber-300">No open period for {{ now()->format('F Y') }}</p>
                    <p class="text-xs text-amber-700 dark:text-amber-400">GL entries cannot be posted without an open accounting period.</p>
                </div>
            </div>
            <button wire:click="createCurrentMonthPeriod"
                    class="ml-4 flex-shrink-0 rounded-lg bg-amber-600 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-700">
                Create {{ now()->format('F Y') }} Period
            </button>
        </div>
    @endif
