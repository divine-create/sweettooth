    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            ...
            <flux:navlist.group :heading="__('Administration')" icon="cog-6-tooth">
                ...
                <flux:navlist.item icon="table-cells" :href="branch_route('branch-dashboard.md-reports.closing-stock-report')"
                    :current="request()->routeIs('branch-dashboard.md-reports.closing-stock-report')" wire:navigate>
                    {{ __('Closing Stock Report') }}
                </flux:navlist.item>
            ...
            <flux:navlist.group :heading="__('Accounting')" icon="currency-dollar" expandable
                ...
                @if ($currentUser->hasAnyRole(['Super Admin', 'MD', 'Managing Director', 'Cost Accountant']) || $sidebarService::isSuperAdmin() || $currentUser->can('view-accounting'))
            ...
            <flux:navlist.group :heading="__('Sales Departments')" icon="building-storefront">
                @forelse($salesDepartments as $dept)
                    <flux:navlist.group :heading="$dept->name" :badge="$dept->category?->name" expandable
                        ...
                        <flux:navlist.item icon="arrows-right-left"
                            :href="branch_route('branch-dashboard.sales-dashboard.transfers.index', [
                                'salesDeptSlug' => $dept->slug,
                                'sales_dept_slug' => $dept->slug,
                                'page' => 'Sales-Point Transfers' . '_' . $dept->slug
                            ])"
                            :current="request()->get('page') === 'Sales-Point Transfers' . '_' . $dept->slug" wire:navigate>
                            {{ __('Transfers') }}
                        </flux:navlist.item>
